-- Schema-only test fixture captured from CSI HIT on 2026-09-23.
-- Never apply this fixture to an existing or production database. No production rows.
...
CREATE TABLE public."clues_base" (
  "id" uuid DEFAULT gen_random_uuid() NOT NULL,
  "title" text NOT NULL,
...
ALTER TABLE public."clues_base" ADD CONSTRAINT "clues_clue_type_check" CHECK ((clue_type = ANY (ARRAY['suspect'::text, 'general'::text, 'free'::text])));
ALTER TABLE public."clues_base" ADD CONSTRAINT "clues_pkey" PRIMARY KEY (id);
...
ALTER TABLE public."clues_base" ADD CONSTRAINT "clues_price_nonnegative" CHECK ((price >= 0));
...
ALTER TABLE public."clues_base" ADD CONSTRAINT "clues_suspect_id_fkey" FOREIGN KEY (suspect_id) REFERENCES suspects(id) ON DELETE SET NULL;
...
ALTER TABLE public."group_clues" ADD CONSTRAINT "group_clues_clue_id_fkey" FOREIGN KEY (clue_id) REFERENCES clues_base(id) ON DELETE CASCADE;
...
ALTER TABLE public."clues_base" ADD CONSTRAINT "clues_category_id_fkey" FOREIGN KEY (category_id) REFERENCES clue_categories(id) ON DELETE SET NULL;
CREATE OR REPLACE FUNCTION public.purchase_clue(target_group_id uuid, target_clue_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
$function$
;
CREATE OR REPLACE FUNCTION public.adjust_group_credits(target_group_id uuid, amount_change integer)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
CREATE OR REPLACE FUNCTION private.get_visible_clues()
 RETURNS TABLE(id uuid, title text, description text, suspect_id uuid, pdf_url text, price integer, is_active boolean, sort_order integer, created_at timestamp with time zone, clue_type text, is_visible boolean, is_global boolean, is_free boolean, file_url text, category_id uuid)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
AS $function$
...
$function$
;
CREATE OR REPLACE FUNCTION private.clue_is_for_own_suspect(target_clue_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
CREATE VIEW public."admin_aankopen_overzicht" WITH (security_invoker=true) AS  SELECT gc.id AS aankoop_id,
   ...
   FROM group_clues gc
     ...
     LEFT JOIN clues_base c ON c.id = gc.clue_id
...
CREATE VIEW public."clues" WITH (security_invoker=true, security_barrier=true) AS  SELECT id,
    title,
    description,
    suspect_id,
    pdf_url,
    ...
    file_url,
    category_id
   FROM private.get_visible_clues() get_visible_clues(id, title, description, suspect_id, pdf_url, price, is_active, sort_order, created_at, clue_type, is_visible, is_global, is_free, file_url, category_id);
...
ALTER TABLE public."clues_base" ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public."clues_base" FROM anon, authenticated;
GRANT ALL ON public."clues_base" TO service_role;
...
REVOKE ALL ON FUNCTION purchase_clue(uuid,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION purchase_clue(uuid,uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION purchase_clue(uuid,uuid) TO service_role;
REVOKE ALL ON FUNCTION is_own_suspect(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION is_own_suspect(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION is_own_suspect(uuid) TO service_role;
REVOKE ALL ON FUNCTION is_test_mode() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION is_test_mode() TO authenticated;
GRANT EXECUTE ON FUNCTION is_test_mode() TO service_role;
REVOKE ALL ON FUNCTION are_final_reports_open() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION are_final_reports_open() TO authenticated;
GRANT EXECUTE ON FUNCTION are_final_reports_open() TO service_role;
REVOKE ALL ON FUNCTION reset_test_data() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION reset_test_data() TO authenticated;
GRANT EXECUTE ON FUNCTION reset_test_data() TO service_role;
REVOKE ALL ON FUNCTION adjust_group_credits(uuid,integer) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION adjust_group_credits(uuid,integer) TO authenticated;
GRANT EXECUTE ON FUNCTION adjust_group_credits(uuid,integer) TO service_role;
REVOKE ALL ON FUNCTION clues_view_delete() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION clues_view_delete() TO service_role;
REVOKE ALL ON FUNCTION clues_view_insert() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION clues_view_insert() TO service_role;
REVOKE ALL ON FUNCTION clues_view_update() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION clues_view_update() TO service_role;
REVOKE ALL ON FUNCTION private.can_read_clue_file(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.can_read_clue_file(text) TO authenticated;
GRANT EXECUTE ON FUNCTION private.can_read_clue_file(text) TO service_role;
REVOKE ALL ON FUNCTION private.get_visible_clues() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.get_visible_clues() TO authenticated;
GRANT EXECUTE ON FUNCTION private.get_visible_clues() TO service_role;
REVOKE ALL ON FUNCTION handle_new_user() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION handle_new_user() TO service_role;
REVOKE ALL ON FUNCTION is_admin() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION is_admin() TO authenticated;
GRANT EXECUTE ON FUNCTION is_admin() TO service_role;
REVOKE ALL ON FUNCTION is_group_member(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION is_group_member(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION is_group_member(uuid) TO service_role;
REVOKE ALL ON FUNCTION private.clue_is_for_own_suspect(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.clue_is_for_own_suspect(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION private.clue_is_for_own_suspect(uuid) TO service_role;
...
CREATE POLICY "clues admin all" ON "public"."clues_base" AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
...
CREATE POLICY "settings deny client access" ON "public"."settings" AS RESTRICTIVE FOR ALL TO authenticated USING (false) WITH CHECK (false);
CREATE POLICY "clue file access if unlocked or admin" ON "storage"."objects" AS PERMISSIVE FOR SELECT TO authenticated USING (((bucket_id = 'clue-files'::text) AND private.can_read_clue_file(name)));
CREATE POLICY "group_clues select own suspect" ON "public"."group_clues" AS PERMISSIVE FOR SELECT TO authenticated USING (( SELECT private.clue_is_for_own_suspect(group_clues.clue_id) AS clue_is_for_own_suspect));
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION handle_new_user();
CREATE TRIGGER clues_view_insert_trigger INSTEAD OF INSERT ON public.clues FOR EACH ROW EXECUTE FUNCTION clues_view_insert();
CREATE TRIGGER clues_view_update_trigger INSTEAD OF UPDATE ON public.clues FOR EACH ROW EXECUTE FUNCTION clues_view_update();
CREATE TRIGGER clues_view_delete_trigger INSTEAD OF DELETE ON public.clues FOR EACH ROW EXECUTE FUNCTION clues_view_delete();
...
CREATE INDEX idx_clues_suspect_id ON public.clues_base USING btree (suspect_id);
CREATE INDEX idx_clues_category_id ON public.clues_base USING btree (category_id);
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.groups, public.clues_base, public.group_clues, public.suspect_notes, public.suspect_statuses, public.notifications, public.agenda_items, public.credit_transactions;
